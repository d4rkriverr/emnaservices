...
func CreateInvoice(db *sql.DB, data Invoice) (int64, error) {
	...
	`
	err := db.QueryRow(query, data.Fullname, data.PhoneNumber, data.ProductName, data.ProductPrice, data.ProductPayment, data.AdvancePayment, data.Agent, data.IssueDate).Scan(&lastInsertId)
	if err != nil {
